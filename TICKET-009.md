...
- [ ] Unit-тесты для OllamaClient: обработка httpx.ConnectError, httpx.TimeoutException, некорректный ответ
- [ ] Unit-тесты для FileManager: append_message, read_messages с обработкой ошибок
- [ ] Unit-тесты для FileManager: обработка несуществующих файлов, повреждённого YAML
- [ ] Unit-тесты для message_handler: обработка пересылок без sender_name (ForwardOriginHiddenUser)
- [ ] Integration-тесты: полный цикл с Ollama недоступен → skip группы, отправка ошибки пользователю
- [ ] Integration-тесты: полный цикл с пустым инбоксом → корректное сообщение пользователю
- [ ] Integration-тесты: обработка больших сообщений (split на части)
- [ ] Integration-тесты: одновременные сообщения от одного пользователя → корректная работа таймеров
- [ ] Integration-тесты: повторная саммаризация → корректное добавление задач без дубликатов
- [ ] E2E-тесты: все команды (/start, /summarize, /inbox, /tasks, /task done, /notes, /settings, /clear)
- [ ] Performance-тесты: обработка большого количества сообщений (>100), много групп
- [ ] Regression-тесты: проверка всех ранее реализованных функциональностей