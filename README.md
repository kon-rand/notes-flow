...
Бот поддерживает Markdown-форматирование ссылок в сообщениях. Когда вы упоминаете номер тикета, бот автоматически создаёт кликабельную ссылку.

**Примеры:**

```
Проверить условия в [LAVKALOGDEV-7797](https://st.yandex-team.ru/LAVKALOGDEV-7797) @k_zahmatов
```

Результат: ссылка на тикет кликабельна и ведёт на `https://st.yandex-team.ru/LAVKALOGDEV-7797`

**Автоматическое добавление ссылок:**

Если вы отправите сообщение без ссылки:
```
Нужно проверить LAVKALOGDEV-7797
```

Бот автоматически преобразует это в:
```
Нужно проверить [LAVKALOGDEV-7797](https://st.yandex-team.ru/LAVKALOGDEV-7797)
```

**Упоминания пользователей:**

Telegram автоматически делает кликабельными упоминания `@username`:
```
Напиши @username по этому вопросу
```

**Технические детали:**

- Формат ссылок: `[ТЕКСТ](URL)` — Markdown
- Формат тикетов: `TICKET-123`, `LAVKAINCIDENTS-1575`
- Базовый URL: `https://st.yandex-team.ru/`
- `parse_mode="Markdown"` используется во всех сообщениях бота
